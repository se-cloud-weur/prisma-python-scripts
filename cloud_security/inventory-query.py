...
def vuln_query(base_url, token, query_param):
    ...
    url = f"https://{base_url}/search/api/v2/config"
...
def main():
    ...
    print('Enter your query i.e.', "config from cloud.resource where api.name = 'azure-vm-list'" )
    query_input = input()
    ...
    full_output = pd.json_normalize(vuln_query_output)
    full_output.to_csv("inv-asset.csv", index=False)
